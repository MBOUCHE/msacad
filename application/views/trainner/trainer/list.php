<div class="content-wrapper py-3">
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="h4 text-center col-sm-12">
                <?php echo mb_strtoupper('Liste des formateurs')?>
        ...
        <div class="row w3-margin-top">
            <div class="col-sm-12 col-md-3 w3-margin-bottom">
                <a href="<?php echo base_url('trainner/trainer/addTrainer')?>" class="w3-btn w3-blue w3-round">Ajouter un formateur</a>
            </div>
        </div>
                ...
                <table class="table table-bordered table-hover small" width="100%" id="dataTable" cellspacing="0">
                    ...
                    if(isset($query) And is_array($query) and !empty($query))
                    {
                        ...
                        for($i = 1; $i <= count($query); $i++)
                        {
                            ...
                            echo '<td id="nameF">' . strtoupper($query[$i-1]->lastname) . ' ' . ucfirst($query[$i-1]->firstname) . '</td>';
                            echo '<td>' . $query[$i-1]->phone .'<br>'. $query[$i-1]->mail . '</td>';
                            echo '<td>
                                    <a href="'.base_url('trainner/trainer/lessons').'/'.$query[$i-1]->id.'" title="Cour(s) dispensé(s)"><i class="w3-btn fa fa-2x fa-list text-info" aria-hidden="true"></i></a>
                                    <a href="'.base_url('trainner/trainer/profile').'/'.$query[$i-1]->id.'" title="Profil"><i class="w3-btn fa fa-2x fa-user w3-text-green" aria-hidden="true"></i></a>
                                    <a href="'.base_url('trainner/trainer/log').'/'.$query[$i-1]->id.'"><i class="w3-btn fa fa-2x fa-bookmark red-tooltip" aria-hidden="true" style="cursor: pointer;" title="Log"></i></a>';
                            switch ($query[$i-1]->locked)
                            {
                                case '0':
                    ...
                    {
                        echo '<tr><td colspan="10"  class="h4 text-center"><a href="'.base_url('admintrainer/save').'" class="text-warning">Aucun formateur enregistré pour le moment ...</a><td></tr>';
                    }
                    ?>
                    </tbody>
                </table>
    ...
    <script type="text/javascript">
        ...
        function unshelve($this){
            ...
            alertify.confirm(
                '<p style="text-align: center;">Voulez vous vraiment suspendre le formateur <br>'
                + '<b>'+$tr.find('td').eq(3).text()+'</b> de matricule : <br>'
                + '<b>'+$tr.find('td').eq(2).text()+'</b> ?'
                + '</p>',
                function(){
                    $(location).attr('href', '<?php echo base_url('trainner/trainer/motivation') ?>'+'/shelve/'+idLa);
                }
        ...
        function shelve($this){
            var idLa = $($this).attr('value'),
                $tr = $($this).parent().parent();
            alertify.confirm(
                '<p style="text-align: center;">Voulez vous vraiment réactiver le formateur <br>'
                + '<b>'+$tr.find('td').eq(3).text()+'</b> de matricule : <br>'
                + '<b>'+$tr.find('td').eq(2).text()+'</b> ?'
                + '</p>',
                function(){
                    $(location).attr('href', '<?php echo base_url('trainner/trainer/motivation') ?>'+'/unshelve/'+idLa);
                }
            ).setHeader('Confirmation de réactivation').set({reverseButtons: true});
        }
