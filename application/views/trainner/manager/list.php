    ...
    <div class="container-fluid">
        <div class="row">
            <div class="h4 text-center col-sm-12">
                <?php echo mb_strtoupper('Liste des gérants') ?>
        ...
        <div class="row w3-margin-top">
            <div class="col-sm-12 col-md-3 w3-margin-bottom">
                <a href="<?php echo base_url('trainner/manager/addManager')?>" class="w3-btn w3-blue w3-round">Ajouter un gérant</a>
            </div>
        </div>
                ...
                <table class="table table-bordered table-hover small" width="100%" id="dataTable" cellspacing="0">
                    ...
                    if(isset($query) And is_array($query) and !empty($query))
                    {
                        ...
                        for($i = 1; $i <= count($query); $i++)
                        {
                            ...
                            echo '<td><a href="'.base_url('trainner/manager/log').'/'.$query[$i-1]->id.'"><i class="w3-btn fa fa-2x fa-bookmark red-tooltip" aria-hidden="true" style="cursor: pointer;" title="Log"></i></a>';
                    ...
                    {
                        echo '<tr><td colspan="6"  class="h3 text-center"><a href="'.base_url('trainner/manager/save').'" class="w3-text-amber">Aucun gérant enregistré pour le moment ...</a><td></tr>';
                    }
                    ?>
                    </tbody>
                </table>
...
<script type="application/javascript">
    ...
    function unshelve($this) {
        ...
        alertify.confirm(
            '<p style="text-align: center;">Voulez vous vraiment suspendre le gérant <br>'
            + '<b>'+$tr.find('td').eq(3).text()+'</b> de matricule : <br>'
            + '<b>'+$tr.find('td').eq(2).text()+'</b> ?'
            + '</p>',
            function(){
                $(location).attr('href', '<?php echo base_url('trainner/manager/motivation') ?>'+'/shelve/'+idLa);
            }
    ...
    function shelve($this) {
        var idLa = $($this).attr('value'),
            $tr = $($this).parent().parent();
        alertify.confirm(
            '<p style="text-align: center;">Voulez vous vraiment activer le gérant <br>'
            + '<b>' + $tr.find('td').eq(3).text() + '</b> de matricule : <br>'
            + '<b>' + $tr.find('td').eq(2).text() + '</b> ?'
            + '</p>',
            function () {
                $(location).attr('href', '<?php echo base_url('trainner/manager/motivation') ?>' + '/unshelve/' + idLa);
            }
        ).setHeader('Confirmation d\'activation ').set({reverseButtons: true});
    }

    $(document).ready(function(){
        alm('collapseGerant', 0);
        <?php
            if(isset($message)){
