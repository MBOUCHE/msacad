 ...
 <?php $i= 1; $j= 1; $point = 0; $M = 0;?>

	<title>TEST DE PREREQUIS</title>
</head>
	<body>
...
<div class="panel panel-default">
  <div class="panel-body">
	<div class="col-md-10" style="margin-left: -26px;">
        <div class="panel panel-default">
          	<div class="panel-body">
            	<ul class="list-group" style="text-align: center;width:100%;">
              		<li class="list-group-item">    	
        				<img src="<?php echo base_url().'assets/img/logo/logo-sm.png' ?>" alt="MULTISOFT ACADEMY" class="rounded float-left" style="width: 53px; height: 53px;margin-left: 4px;">
		              	<h3>TEST DE PREREQUIS POUR LA FORMATION : 
		              	<?php 
		              		$all_lesson = $this->db->select('*')->from('lesson')->where('id', $id)->get()->result_array();
		              		foreach ($all_lesson as $key) {
		              			if ($key['id'] =$id) {
		              				$label=$key['label'];
		              				echo mb_strtoupper($label).' ( '.$key['code'].' )';
		              			}
		              		}
		              	?></h3>
...
?>
            <form action='<?php echo base_url().'e_controllers/c_home_page/correctionTest/'.$id ;?>' method="post">
              ...
              <?php
	 				foreach ($query as $key2) {
	 					...
	 					$all_qsts = $this->db->select('*')->from('e_question')->where('id_exercise', $key2['id_exercise'])->order_by('answer', 'RANDOM')->get()->result_array();
	 					if ($key2['id_exercise']) {
		 					echo '<div class="panel panel-info" style="margin-left:17px;width:97%;">
						...
						foreach ($all_qsts as $key301) {
		 				...
		 				foreach ($all_qsts as $key5) {
		                    ...
		                    if ($k < 3) {
		                        echo '<div class="panel panel-success" style="border-radius:0px;margin-left: -13px; width: 107%;">
		        ...
		        <div class="row">
					<div class="input-group col-md-9">
					  <span class="input-group-addon" id="sizing-addon2" style="width: 296px;">Votre avis sur ce test</span>
					  	<input type="text" class="form-control" name="note" placeholder="Bien vouloir entrer une remarque." aria-describedby="sizing-addon1">
					</div>
					<div class="col-md-3" style="margin-left: -400px">
			          	<button class="btn btn-success" style="width: 202px; margin-left: 404px;" type="submit">
  	...
  	<div class="col-md-2" style="position: fixed; margin-left: 79.5%">
		<div class="panel panel-default" style="margin-left: -22px; width: 260px;">
			...
			?>
	  		...
	  		<div class="panel-body" style="background-color: #fedb95;" id="countdown">
		    	<li class="list-group-item">
		    		Total de points : <?php echo $point; ?> points
		    	</li>
		    	<li class="list-group-item">
		    		Constituée le : <?php  echo moment($key2['last_modify'])->format('d/m/Y <br> à H:m:s').' .';?>
		    	</li>
		    	<li class="list-group-item">
		    		Temps alloué : <?php echo ' '.$key2['duration'].' '; ?> minutes
			  		...
			  		<div class="panel-footer">Constituée par : <br>M
			  		<?php 
