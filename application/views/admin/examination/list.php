    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-sm-12 table-responsive">
                <table class="table table-bordered table-hover small" width="100%" id="" cellspacing="0">
                    <thead>
                    <tr>
                    ...
                    if ($results!=null)
                        foreach ($results as $result)
                        {
                            $line++;
                            echo "<tr>";
                            $nbrLine=count($result->results);
                            echo "<td rowspan='$nbrLine'>".$line."</td>";
                            echo "<td rowspan='$nbrLine' class='small'>".mb_strtoupper($result->promotion->label);
                            echo "<br><br>";
                            if (!$result->allPub)
                            ...
                            if ($result->allMarked)
                                echo "<a href='".base_url('admin/examination/results/')."/".$result->promotion->code."/".permalink('all')."' class='showresult w3-btn w3-white w3-border w3-border-blue w3-margin-small' title='Consulter tous les résultats'><i class='fa fa-list fa-2x w3-text-blue'></i>  Consulter tous les résultats</a>";
                            echo "</td>";
                            echo "<td rowspan='$nbrLine' class='small'>".$result->promotion->code."</td>";
                            if ($result->results!=null)
                                foreach($result->results as $k=>$rs)
                                {
                                    ...
                                    echo "<td>
                                      ...
                                      <a href='".base_url('admin/examination/results/')."/".$rs->pcode."/".permalink($rs->ecode)."' class='showresult w3-btn w3-blue small w3-margin-small'>Consulter</a>";
                                    echo "</td>";
                                    echo "</tr>";
                                }
                        }
    ...
    $(document).ready(function(){
        ...
        $('.publish').on('click', function(){
            ...
            alertify.confirm("Confirmation de publication", "Voulez-vous vraiment publier les résultats de cette évaluation ?",
                function(){
                    window.location.href='<?php echo base_url('admin/examination/publish/')?>'+'/'+id.toString()+'/'+code.toString();
                },
                function(){
        ...
        $('.publishAll').on('click', function(){
            ...
            alertify.confirm("Confirmation de publication", "Voulez-vous vraiment publier les résultats de cette vague ?",
                function(){
                    window.location.href='<?php echo base_url('admin/examination/publish/')?>'+'/'+id.toString();
                },
                function(){
                    alertify.error("Publication annulée");
                });
