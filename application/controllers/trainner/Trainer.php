...
class Trainer extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('trainner/trainer_model', 'trainer');
    ...
    public function formAdd()
    {
        ...
        if($this->form_validation->run())
        {
            ...
            if(is_bool($postOk) And $postOk)
            {
                ...
                redirect("trainner/trainer");
            }
            else
            {
                $this->data['status']=false;
                $this->data['message'] = '<b>Echec d\'enregistrement:</b> '.$postOk;
                $this->render('trainner/trainer/form-add', 'Enregistrer un formateur');
            }
        }
        else
        {
    ...
    public function all()
    {
        ...
        $this->render('trainner/trainer/list', 'Liste des formateurs');
    }
    ...
    public function allocation($id=null)
    {
        ...
        if($this->form_validation->run()) {
            ...
            else
            {
                ...
                $this->render('trainner/trainer/allocation', 'Allocation à un enseignement');
            }
        }
        else
        {
    ...
    {
        $this->render('trainner/trainer/addTrainer', 'Ajout d\'un formateur');
    }
    ...
    public function profile($id = false, $print = false){
        if(isset($id) and $print != 'print'){
            ...
            $this->render("trainner/trainer/profile", "Profil Professionnel de ".strtoupper($trainer->lastname).' '.ucfirst($trainer->firstname));
            ...
            $content = $this->load->view('trainner/trainer/pdf-profil', $this->data, TRUE);
    ...
    public function log($id=false){
        ...
        $this->data['log'] = $log;
        $this->render('trainner/trainer/log', 'Liste des logs');
    }
        ...
        if($idF){
            ...
            if(!empty($user)){
                ...
                if(!empty($lessonDispense)){
                    ...
                    $this->render('trainner/trainer/lessons-list', 'Liste des enseignements dipensés par '.$user->firstname.' '.$user->lastname);
                }else{
    ...
    public function shelve($idLa=false){
        if($idLa){
            ...
            if(is_array($trainer) and is_array($lesson)){
                ...
                if($shelve){
                    $this->load->model('trainner/log_model', 'logs');
                    ...
                    if($log){
                        ...
                        redirect("trainner/trainer/lessons/".$trainer[0]->id);
                    }
                }else{
    ...
    public function motivation($mode = false, $idUr = false){
        if(($mode=="shelve" or $mode == "unshelve") and $idUr){
            ...
            if(is_array($trainer)){
                ...
                $this->data['idUr'] = $idUr;
                //$this->data['mode'] = $mode;
                $this->render('trainner/trainer/motivation', 'Enregistrer la motivation');
            }else{
                redirect('trainner/trainer/all');
            }
        }
    ...
    public function allLessonSlip(){
       $this->load->model('trainner/availability_model');
        ...
        if(empty($lessonDispense)){
            $this->render('trainner/trainer/LessonSlip-list', 'Fiche de suivis des formateurs');
        }else{
    ...
    {
        $this->load->model('trainner/session_model', 'session');
        $this->load->model('trainner/period_model', 'period');
        ...
        if ($sess==null)
        {
            ...
            if($this->form_validation->run())
            {
                ...
                redirect('trainner/trainer/allLessonSlip');
            } else
            {
                redirect("trainner/trainer/edit/$session");
            }
        } else
        {
            ...
            $this->render('trainner/trainer/lesson-slip-edit', 'Editer une fiche de suivi');
        }
    }
