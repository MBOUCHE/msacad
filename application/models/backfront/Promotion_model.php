...
class Promotion_model extends CI_Model
{
    ...
    public function getAllocatedPromo($id, $state="")
    {
        if ($state!='')
        {
            ...
            WHERE promotion.state=? AND lesson_allocation.user=?
            ORDER BY promotion.id DESC', array($state, $id));
        } else
        {
            $query=$this->db->query('
            SELECT promotion.*, lesson.label 
            ...
            LEFT JOIN lesson ON promotion.lesson = lesson.id 
            LEFT JOIN lesson_allocation ON promotion.lesson = lesson_allocation.lesson 
            WHERE lesson_allocation.user=?
            ORDER BY promotion.id DESC', $id);
        }
    ...
    public function getStudentPromotions($id, $state="")
    {
        ...
        {
            $query=$this->db->query('
            SELECT DISTINCT(promotion.id),  promotion.*, lesson.label
            FROM promotion
            LEFT JOIN lesson ON promotion.lesson = lesson.id
            LEFT JOIN registration ON promotion.id = registration.promotion
