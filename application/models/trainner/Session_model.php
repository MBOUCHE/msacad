...
class Session_model extends  CI_Model
{
    ...
    public function saveTimetable($timetable, $start_date, $end_date, $distinctPromos)
    {
        ...
        foreach ($timetable as $curClass)
        {
            ...
            $nbrProg=$this->db->query('select id from sessions where promotion=?', $curClass->promotion)->num_rows();
             $proms = explode('#',$curClass->promotion);
            array_pop($proms);
            array_shift($proms);
            $lastCodeString = "";
            if(count($proms) > 1){
                $lastCodeString = "#".join('',$proms);
            }
            else{
                $lastCodeString = $this->availability->getCode($proms[0]);
            }
            //var_dump($lastCodeString);die();

            $curSession=array('SESS'.castNumberId($nbrProg+1).$lastCodeString, $curClass->promotion, $sd, $curClass->day ,$curClass->period, $period->end-$period->start);
            //var_dump($curSession);
        ...
        {
            $this->notification->publish(array("sender"=>$this->userId, "content"=>"L'emploi du temp de la semaine du ".$start_date." au ".$end_date." est disponible. Le cours de ".$this->availability->getLesson($pr)." a été programmé.", "send_date"=>moment()->format('Y-m-d H:i:s'), "target"=>STUDENT, "promotion"=>$pr, "url"=>"none"));
        }
    ...
    {
        $query=$this->db->query('select * from sessions where id=?', intval($session));
        return !empty($query->result())?$query->result()[0]:null;
    }

    public function update($trainer, $session)
    {
        return $this->db->query('update sessions set user=? where id=?', array($trainer, $session));
    }
}