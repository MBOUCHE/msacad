...
class Admin_model extends CI_Model
{
    ...
    public function save($data = array())
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $message=($user->sexe==0?'Mme ':'M. ').mb_strtoupper($user->lastname).' '.ucwords($user->firstname).", Votre inscription a bien &eacute;t&eacute; enregistr&eacute;e. Veuillez utiliser les param&egrave;tres suivants pour <a href='".base_url('trainner/trainner/save')."'>activer votre compte</a> : <br>Votre login : <b>" . $data['mail'] . "</b><br>Votre mot de passe : <b>" . $data['pwd'] . ".";
    ...
    {
        return $this->db->query('select user.* from user inner join user_role on user.id=user_role.user where user.id=? and (user_role.role=?)', array($id, '3'));
    }
    ...
    public function modify($data=array())
    {
        ...
        $this->notification->publish(array(
            ...
            "target"=>ADMIN,
            "promotion"=>"",
            "url"=>base_url('trainner/loggedUserProfile/').$this->userId
        ));
        ...
        $this->notification->publish(array(
            ...
            "target"=>MANAGER,
            "promotion"=>"",
            "url"=>'trainner/loggedUserProfile/'.$this->userId
        ));
