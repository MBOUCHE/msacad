...
class Examination_model extends CI_Model
{
    ...
    public function getResults($promotion)
    {
        $query=$this->db->query('select evaluation_results.promotion, evaluation_results.ready, evaluation_results.published, evaluation_results.date, evaluation_results.evaluation, lesson.label as l_label, promotion.code as pcode, evaluation.label as e_label, evaluation.code as ecode
                                      from evaluation_results
                                      left join promotion
                                      on evaluation_results.promotion=promotion.id
                                      left join lesson
                                      on promotion.lesson = lesson.id
                                      inner join evaluation on lesson.id = evaluation.lesson
                                      and evaluation_results.evaluation=evaluation.id
                                      where evaluation_results.promotion=?
                                      ', $promotion);
        return !empty($query->result()) ? $query->result() : null;
    }
